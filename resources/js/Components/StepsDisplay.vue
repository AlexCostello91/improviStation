      ...
      <ol role="list" class="flex items-center">
        <li v-for="(step, stepIdx) in steps" :key="step.name" :class="[stepIdx !== steps.length - 1 ? 'pr-8 sm:pr-20' : '', 'relative']">
          <template v-if="step.status === 'complete'">
            ...
            <a @click.prevent="$emit('goToStep',step.step-1)" class="relative flex h-8 w-8 items-center justify-center rounded-full bg-indigo-600 hover:bg-indigo-900">
          ...
          <template v-else-if="step.status === 'current'" condition="step.status === 'current'">
            <div class="absolute inset-0 flex items-center" aria-hidden="true">
              <div v-if="step.step < 6 && steps[step.step].status==='complete'" class="h-0.5 w-full bg-indigo-600" />
              <div v-else class="h-0.5 w-full bg-gray-200" />
          ...
          <template v-else>
            ...
            <a @click.prevent="$emit('goToStep',step.step-1)" href="#" class="group relative flex h-8 w-8 items-center justify-center rounded-full border-2 border-gray-300 bg-white hover:border-gray-400">
